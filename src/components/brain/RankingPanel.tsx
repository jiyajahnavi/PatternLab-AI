...
const TierLadder: React.FC<{ currentRating: number }> = ({ currentRating }) => {
  ...
  return (
    <div className="w-80 shrink-0 flex flex-col h-full">
      <div className="text-[10px] font-bold text-muted uppercase tracking-widest mb-4 text-right">Progression Ladder</div>
          ...
          return (
            <div
              ...
              className={`flex items-center justify-between px-3 py-2.5 rounded-xl transition-all duration-300 ${
                isCurrent
                  ? `${tier.bg} border border-current/40 scale-[1.03] shadow-lg shadow-black/40 z-10`
                  : isUnlocked
                  ? 'bg-background/20 border border-border/20 opacity-60'
                  : isNext
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                <span className={`text-sm leading-none shrink-0 ${isCurrent ? 'animate-pulse' : ''}`}>{tier.icon}</span>
                <span className={`text-[11px] font-black tracking-tight truncate ${isCurrent ? tier.color : isUnlocked ? 'text-primary' : 'text-muted'}`}>
                  {tier.tier}
                </span>
              </div>
              <div className="shrink-0 text-[9px] font-mono font-bold opacity-40 ml-2">
                {tier.min}
              </div>
            </div>
          );
...
export const GlobalRankingPanel: React.FC = () => {
  ...
  return (
    <div className="bg-surface border border-border rounded-2xl p-7 relative overflow-hidden shadow-xl shadow-black/20 min-h-[700px] flex flex-col">
      <div className="absolute top-0 right-0 w-64 h-64 bg-violet-500/5 rounded-full blur-3xl pointer-events-none" />

      <div className="grid grid-cols-1 lg:grid-cols-[1fr,320px] gap-8 relative items-stretch">
        ...
        <div className="min-w-0 space-y-6">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-bold text-primary">Global Standing</h3>
              <p className="text-[10px] text-muted">Intelligent skill-based ranking</p>
            </div>
          </div>
        ...
        {/* Vertical Divider */}
        <div className="absolute right-[320px] top-0 bottom-0 w-px bg-border/40 hidden lg:block" />
...
export const ActivityPanel: React.FC = () => {
  ...
  const handleOpenProblem = (s: any) => {
    if (s.fullQuestion) {
      setProblem(s.fullQuestion);
      navigate(`/problem/${s.questionId}`);
    }
  };

  return (
    <div className="bg-surface border border-border rounded-2xl p-6 relative overflow-hidden shadow-xl shadow-black/20">
      <div className="flex items-center gap-2 mb-4">
        <HistoryIcon size={14} className="text-violet-400" />
        <h3 className="text-sm font-bold text-primary">Recent Activity</h3>
        <span className="ml-auto text-[10px] text-muted">{sessions.length} total sessions</span>
      </div>
      {sessions.length === 0 ? (
        <div className="h-40 flex items-center justify-center border border-dashed border-border/40 rounded-2xl p-8 text-center bg-background/20">
          <p className="text-[10px] text-muted italic leading-relaxed">No sessions recorded yet.<br/>Generate a question to build your profile!</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
          {sessions.map(s => {
            ...
            return (
              <div 
                key={s.id} 
                onClick={() => handleOpenProblem(s)}
                className="group flex items-center gap-3 p-3 rounded-xl bg-background/40 border border-border/50 hover:border-violet-500/50 hover:bg-violet-500/5 transition-all cursor-pointer active:scale-[0.98]"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 text-[9px] text-muted/60 mt-0.5">
                    ...
                    <span>·</span>
                    <span className="text-[8px] opacity-70 uppercase tracking-tighter">{new Date(s.timestamp).toLocaleDateString()}</span>
                  </div>
                </div>
